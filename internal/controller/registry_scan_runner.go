...
func (r *RegistryScanRunner) Start(ctx context.Context) error {
	...
	log.Info("Starting registry scan runner", "interval", r.ScanInterval)
...
func (r *RegistryScanRunner) checkRegistryForScan(ctx context.Context, registry *v1alpha1.Registry) error {
	...
	if !lastScanJob.IsComplete() && !lastScanJob.IsFailed() {
		log.V(1).Info("Registry has a running ScanJob, skipping.", "registry", registry.Name, "status conditions", lastScanJob.Status.Conditions)
...
func (r *RegistryScanRunner) SetupWithManager(mgr ctrl.Manager) error {
	if err := mgr.GetFieldIndexer().IndexField(context.Background(), &v1alpha1.ScanJob{}, "spec.registry", func(rawObj client.Object) []string {
		...
		return fmt.Errorf("failed to index ScanJob by registry: %w", err)
	}
