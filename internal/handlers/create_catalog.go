...
func (h *CreateCatalogHandler) Handle(ctx context.Context, message []byte) error { //nolint:gocognit,funlen // We are a bit more tolerant for the handler.
	...
	if !scanJob.IsInProgress() {
		h.logger.WarnContext(ctx, "ScanJob is not in progress, skipping catalog creation", "scanjob", scanJob.Name, "namespace", scanJob.Namespace)
		return nil
	}
	...
	for _, image := range imageToProcess {
		h.logger.DebugContext(ctx, "Send sbom message", "image", image.Name)
		...
		if err != nil {
			return fmt.Errorf("cannot marshal generate sbom message for image %s: %w", image.Name, err)
		}

		if err = h.publisher.Publish(ctx, GenerateSBOMSubject, messageID, message); err != nil {
			return fmt.Errorf("cannot publish generate sbom message for image %s: %w", image.Name, err)
		}
	}
